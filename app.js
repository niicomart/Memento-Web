...
function abrirDetalleElemento(elemId) {
  ...
  document.getElementById('modalDetalle').classList.remove('keyboard-open');
  sincronizarScroll();
...
function abrirModal(titulo, html, onConfirm, confirmText='Guardar') {
  ...
  mod.classList.remove('keyboard-open');
...
// Scroll de fondo fijo mientras haya un modal abierto (evita saltos de tamaño en la página)
...
// El modal se adapta a la altura del teclado virtual: sube y se encoge
// hasta ocupar solo la franja visible encima del teclado.
function initTeclado(){
  ...
  const actualizar=()=>{
    ...
    if(!vv) return;
    const kbAbierto = (window.innerHeight - vv.height) > 60;
    mod.classList.toggle('keyboard-open', kbAbierto);
    det.classList.toggle('keyboard-open', kbAbierto);
    if(kbAbierto){
      const visibleTop = Math.max(0, Math.round(vv.offsetTop));
      const altoVisible = Math.max(180, Math.round(vv.height));
      raiz.style.setProperty('--kb-top', visibleTop + 'px');
      raiz.style.setProperty('--kb-h', altoVisible + 'px');
    } else {
      raiz.style.removeProperty('--kb-top');
      raiz.style.removeProperty('--kb-h');
    }
  };
...
function pedirDatoRapido(opciones){
  ...
  return new Promise(resolve=>{
    ...
    abrirModal(titulo, html, null, 'Guardar');
...
// Mensaje de error visible dentro del modal abierto (reemplaza alert() de validación)
...
function abrirModalElemento(elemId=null){
  ...
  `;
  abrirModal(esEdicion?'Editar elemento':'Nuevo elemento', html, async ()=>{
    const nombre=document.getElementById('field_nombre').value.trim();
    if(!nombre){ mostrarErrorModal('Nombre requerido'); document.getElementById('field_nombre').focus(); return; }
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  document.addEventListener('keydown', (e)=>{
    if(e.key==='Escape'){ cerrarModal(); cerrarDetalle(); const fm=document.getElementById('fabMenu'); if(fm) fm.classList.add('hidden'); }
